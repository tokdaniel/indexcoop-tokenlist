...
| `is[Type]Token` | `:is [TokenType]` | typeguard to safely cast into the necessary tokentype: `TokenInfo`, `ListedToken`, `IndexToken`, `ProductToken`,  `LeverageToken`, `SectorToken`, `YieldToken` |
...
| `getChainProductTokenList` | `:ProductToken[]` | a function requiring a `chainId` of type `number` or `ChainId`. Returns all product tokens if Index Cooperative in an array on the provided chain. If the chain is unsupported, it will return an empty array, however if the provided types are exact, it will return the right tokens by type. | 
