...
| `is[Type]Token` | `:is [TokenType]` | typeguard to safely cast into the necessary tokentype: `TokenInfo`, `ListedToken`, `IndexToken`, `ProductToken`,  `LeverageToken`, `SectorToken`, `YieldToken`, `CurrencyToken`, `ComponentToken` |
...
| `getChainProductTokenList` | `:ProductToken[]` | a function requiring a `chainId` of type `number` or `ChainId`. Returns all product tokens in an array on the provided chain. If the chain is unsupported, it will return an empty array, however if the provided types are exact, it will return the right tokens by type. |
| `getChainCurrencyTokenList` | `:CurrencyToken[]` | a function requiring a `chainId` of type `number` or `ChainId`. Returns all currency tokens in an array on the provided chain. If the chain is unsupported, it will return an empty array, however if the provided types are exact, it will return the right tokens by type. |
| `getChainComponentTokenList` | `:ComponentToken[]` | a function requiring a `chainId` of type `number` or `ChainId`. Returns all component tokens in an array on the provided chain. If the chain is unsupported, it will return an empty array, however if the provided types are exact, it will return the right tokens by type. |
